...
public class PlayerController : MonoBehaviour
{
    ...
    private void SetPlayerAnimationSpeed()
    {
        ...
        player.animator.speed = movementSpeed / Settings.baseSpeedForPlayerAnimations;
    }
    ...
    private void MovementInput()
    {
        ...
        bool isLeftCtrlClicked = Input.GetKeyDown(KeyCode.LeftControl);
        bool isLeftShiftBeingClicked = Input.GetKey(KeyCode.LeftShift);

        if (isLeftShiftBeingClicked && playerStatus.GetCurrentStamina() > 0f)
        {
            movementSpeed = runSpeed;

            //Call run event in order to decrease the stamina over time
            player.runEvent.CallRunEvent();
        }
        else
        {
            //Call not run event in order to decrease the stamina over time
            player.runEvent.CallOnNotRun();
        ...
        {
            if (!isLeftCtrlClicked)
            {
                // trigger movement event
                player.movementByVelocityEvent.CallMovementByVelocityEvent(direction, movementSpeed);
            }
            // else player roll if not cooling down
            else if (playerRollCooldownTimer <= 0f && playerStatus.IsThereStaminaToAction(20))
            {
    ...
    private IEnumerator PlayerRollRoutine(Vector3 direction)
    {
        ...
        playerStatus.DecreaseStamina(20);

        while (Vector3.Distance(player.transform.position, targetPosition) > minDistance)
        {
            player.movementToPositionEvent.CallMovementToPosition(targetPosition, player.transform.position, movementDetails.GetRollSpeed(), direction, isPlayerRolling);
        ...
        // Set cooldown timer
        playerRollCooldownTimer = movementDetails.rollCooldownTimer;
    ...
    /// </summary>
    private void AimThroughSightInput()
    {
        int NoScopeGunAimWeight = 2;
        int NoScopeGunDefaultWeight = 1;

        bool hasGunScope = player.activeWeapon.GetCurrentWeapon().weaponDetails.hasScope;
    ...
    private void AimWeaponInput(out Vector3 weaponDirection, out float weaponAngleDegrees, out float playerAngleDegrees, out AimDirection playerAimDirection)
    {
        ...
        Vector3 mouseWorldPosition = UtilsClass.GetWorldMousePosition();
    ...
    {
        UtilsClass.ValidateCheckNullValue(this, nameof(movementDetails), movementDetails);
    }
